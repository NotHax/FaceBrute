#!/bin/bash
#/////////////////////////////////////////////////////////////////////////////////////////////
#/////////////////////////////////////////////////////////////////////////////////////////////
#////                                                                                     ////
#////  _   _           _      _____                              _   _                    ////
#//// | \ | |         | |    / ____|                            | | (_)                   ////
#//// |  \| |   ___   | |_  | |  __   _   _    __ _   _ __    __| |  _    __ _   _ __     ////
#//// | . ` |  / _ \  | __| | | |_ | | | | |  / _` | | '__|  / _` | | |  / _` | | '_ \    ////                        
#//// | |\  | | (_) | | |_  | |__| | | |_| | | (_| | | |    | (_| | | | | (_| | | | | |   ////
#//// |_| \_|  \___/   \__|  \_____|  \__,_|  \__,_| |_|     \__,_| |_|  \__,_| |_| |_|   ////
#////                                                                                     ////
#/////////////////////////////////////////////////////////////////////////////////////////////
#/////////////////////////////////////////////////////////////////////////////////////////////

g="\033[32m"
b="\033[1;34m"
c="\033[1;36m"
r="\033[1;31m"
y="\033[1;93m"
h="\033[0;30m"
clear
echo $y "
$b::::::::::     :::      ::::::::  :::::::::: $r:::::::::  :::::::::  :::    ::: ::::::::::: ::::::::::
$b:+:          :+: :+:   :+:    :+: :+:        $r:+:    :+: :+:    :+: :+:    :+:     :+:     :+:
$b+:+         +:+   +:+  +:+        +:+        $r+:+    +:+ +:+    +:+ +:+    +:+     +:+     +:+
$b:#::+::#   +#++:++#++: +#+        +#++:++#   $r+#++:++#+  +#++:++#:  +#+    +:+     +#+     +#++:++#
$b+#+        +#+     +#+ +#+        +#+        $r+#+    +#+ +#+    +#+ +#+    +#+     +#+     +#+
$b#+#        #+#     #+# #+#    #+# #+#        $r#+#    #+# #+#    #+# #+#    #+#     #+#     #+#
$b###        ###     ###  ########  ########## $r#########  ###    ###  ########      ###     ########## v1.0

$g╔═════════════════════════════════════════════════╗
&g║ $cTools     $y: $g₣aceBruteV1.0               
$g║ $cAuthor    $y: $gNotGuardian
$g║ $cGithub    $y: $ghttps://github.com/NotHax
$g║ $cInstagram $y: $ghttps://instagram.com/notguardianyt
$g║ $yWe From ChipWy (Chicken Playing With You) 
$g║ $yJika Ada Bug Tinggal DM Author Aja Ya
$g╚═════════════════════════════════════════════════╝"
echo $c " [$y 01 $c] $g Mulai Cracking "
echo $c " [$y 02 $c] $g Buat WorldList "
echo $c " [$y 00 $c] $r Keluar"
echo $y "Pilih 1/2/3?"
read -p "" pil;

case $pil in
01)echo $y "Memilih Nomor 01"
sleep 1
clear
python2 FaceBrute.py

;;

1)clear
 echo "Memilih Nomor 1"
sleep 1
clear
python2 FaceBrute.py

;;

02) clear
echo "Memilih Nomor 02"
sleep 1
echo "Tips Buat WorldList Adalah Tempat Password Target"
sleep 2
nano WorldList
echo $y "WorldList Tersimpan"
sleep 1
clear
sh FCB.sh

;;

2)  clear
echo "Memilih Nomor 2"
sleep 1
echo "Tips Buat WorldList Adalah Tempat Password Target"
sleep 2
nano WorldList
echo $y "WorldList Tersimpan"
sleep 

clear
sh FCB.sh

;;

00)echo "Memilih Nomor 00"
sleep 1
echo $y"Sampai Jumpa Lagi"
sleep 1
exit

;;

0)echo $y "Memilih Nomor 0"
sleep 1
echo $y "Sampai Jumpa Lagi"
sleep 1
exit

;;

*) echo $y "Maaf encarian Anda Tidak Ditemukan"
sleep 2.1
esac
done
done
